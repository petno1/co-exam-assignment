function [xopt,lambdaopt,Wset,it] = qpsolverActiveSet(H,g,A,b,Aeq,beq,x0) 
...
nablaxL = gk - A*lambda;
c = A'*x + b;                   % c(x) = A' x + b >= 0
ceq = Aeq'*x - beq;             % ceq(x) = Aeq' x - beq = 0

% Check if the initial point is optimal
KKTstationarity = ( norm(nablaxL,'inf') < tol ) && ( norm(ceq,'inf') < tol );
...
while ( ~KKTconditions && (it < maxit) );
    ...
    [p,lambdaWset] = EqualityQPsubproblem(H,gk,Aw,b,Aeq,beq);
   ...
   if ( norm(p,'inf') > tol ) % p is non-zero
        ...
        nIWset = size(IWset,1);
        for i = 1:nIWset
            if IWset(i) <= m % inequality constraint
                pA = A(:,IWset(i))'*p;
                if pA < 0.0
                ...
                pAeq = Aeq(:,IWset(i)-m)'*p;
                if abs(pAeq) > tol
                    alphaeqpAeq = - ceq(IWset(i)-m,1)/pAeq;
                    if alphaeqpAeq < alpha
                        alpha = alphaeqpAeq;
                        idc = i;
                    end
                end
