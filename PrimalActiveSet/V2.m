function [xopt,lagrangeopt,Wset,it] = V2(H, g, A, b, Cbar, dbar, x0)
...
% QP data
gk = H*x + g;
nablaxL = gk - Cbar*lambda
c = Cbar'*x + dbar;                   % c(x) = Cbar' x + dbar >= 0

% Check if the initial point is optimal
KKTstationarity = ( norm(nablaxL,'inf') < tol);
...
while ( ~KKTconditions && (it < maxit) );
    ...
    [p,lambdaWset] = EqualityQPsubproblem(H,gk,Aw,bw,A,b);
